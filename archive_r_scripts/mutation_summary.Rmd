...
dt = dt %>%
  group_by(Domain) %>%
  summarise(
    Mutations = length(unique(pattern)),
...
```{r cp_muts}
dt = read.dbTable('table_cp_muts.csv', tables_dir=TABLEDIR)
styledDT(dt)
```

### VP by reference (mutations)
```{r vp_muts}
dt = read.dbTable('table_vp_muts.csv', tables_dir=TABLEDIR)
styledDT(dt)
```

### CP by reference (variants)
```{r cp_variants}
dt = read.dbTable('table_cp_variants.csv', tables_dir=TABLEDIR)
styledDT(dt)
```

### VP by reference (variants)
```{r vp_variants}
dt = read.dbTable('table_vp_variants.csv', tables_dir=TABLEDIR)
styledDT(dt)
```
