...
draw_titer_by_variant <- function(dfPlot, figLabel) {
  ...
  p <- ggplot(data=dfPlot, aes(x=variant, y=titer, fill=variant)) +
    ...
    scale_y_continuous(
      trans='log2',
      expand=expansion(mult = c(0, .2)),
      breaks=c(64, 128, 256, 512, 1024, 2048, 4096),
      limits=c(64, 10000)
    ) +
    scale_x_discrete(breaks=VARIANT, labels=VARIANT) +
    geom_text(
      data=dfAgg,
      aes(y=10000, label=label),
      size = 7
    ) +
    theme(
      legend.position = "none",
...
draw_titer_pcnt <- function(dfPlot, figLabel) {
  ...
  p <- ggplot(data=dfPlot, aes(x=variant, y=num_result, fill=level)) +
...
```{r titer-fold-point, fig.height=20, fig.retina=4, fig.width=20, message=FALSE, warning=FALSE, echo=FALSE}


draw_titer_fold_points <- function(dfPlot, figLabel='') {
  
  p <- ggplot(data=dfPlot, aes(x=titer, y=fold, color=rx_name)) +
    geom_point(size=3) +
    scale_y_continuous(
      trans="log10",
      expand=expansion(mult = c(0, .2)),
...
p = draw_titer_fold_points(titer_df)
p
```
