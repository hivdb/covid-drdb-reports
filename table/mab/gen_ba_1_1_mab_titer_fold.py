...
WHERE
    ...
    AND
    test_iso.var_name = 'Omicron/BA.1.1'
...
def gen_ba_1_1_mab_titer_fold(
    ...
    dump_csv(
        (
            DATA_FILE_PATH / 'mab' /
            'omicron_ba_1_1_mab_titer_fold_forest_figure.csv'
        ),
        save_results)
    ...
    calc_median_iqr(
        save_results, (
            DATA_FILE_PATH / 'mab' /
            'omicron_ba_1_1_mab_median_iqr.csv'
        ))
...
def report_virus_type(records):
    ...
    for mab, mab_rec_list in mab_group.items():
        ...
        if 'num_AV' not in report_rec:
            report_rec['num_AV'] = ''
            report_rec['AV'] = ''
        if 'num_PV' not in report_rec:
            report_rec['num_PV'] = ''
            report_rec['PV'] = ''
