...
```{r plasma-fold-fig, fig.height = 10, fig.width = 14, fig.retina = 4, message = FALSE}
...
dfForPlot = dfForPlot %>%
  ...
  filter(ref_rx %notin% excludedRefRxNames) %>%
  mutate(
...
dfNumRefs = dfForPlot %>%
  ...
  summarize(num_refs = length(unique(ref_name))) %>%
  group_by(strain_name, in_trial) %>%
  summarize(num_refs = max(num_refs))
