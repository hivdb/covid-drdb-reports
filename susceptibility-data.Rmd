...
dfSuscResultsMAb = dfSuscResultsMAb %>%
  filter(control_strain_name %in% ControlStrainNames)
...
dfSuscResultsCP = dfSuscResultsCP %>%
  filter(control_strain_name %in% ControlStrainNames) %>%
  filter(variant %in% CPVariantNames)
...
authenticStrains = c(
  ...
  "B.1.429 authentic" = "CAL20C Spike",
  "B.1.427 authentic" = "CAL20C Spike"
)
...
strainTitle = c(
  ...
  "P.1 Spike" = "P.1",
  "CAL20C Spike" = "CAL.20C (S:S13I+S:W152C+S:L452R)"
  #"S:417N" = "417N",
  #"S:439K" = "439K",
  #"S:452R" = "452R",
  #"S:453F" = "453F"
...
dfForPlot = dfSuscResultsPlasma %>%
  mutate(
    strain_name = ifelse(
      strain_name %in% names(authenticStrains),
      yes = authenticStrains[strain_name],
      no = strain_name
    )
  ) %>%
  ...
  group_by(ref_name, rx_name, vaccine_name, strain_name, resistance_level, group, assay) %>%
  dplyr::summarise(
  ...
  for (groupName in c("CP", "IP")) {
    ...
    plot = dfGroup %>%
      ...
      geom_bar(
        ...
        size = 0.2,
        color = "#000000",
        stat = 'identity', position = 'stack', width = 0.06 * numItems) +
      scale_x_discrete(
    ...
    if (i %% 2 == 1) {
      plot = plot + annotate(
          ...
          y=Inf,
          x=0,
          label=paddingTitle(strainTitle[[strain]]),
          size=3,
          vjust=1.5,
          hjust=0
          )
...
dfForPlot = dfSuscResultsMAb %>%
  ...
  select(ref_name, ab_name, strain_name, in_trial, resistance_level, fold_cmp, fold) %>%
  group_by(ref_name, ab_name, strain_name, in_trial, fold_cmp) %>%
  dplyr::summarise(
    fold = max(fold),
...
dfNumAbs = dfForPlot %>%
  group_by(strain_name, in_trial) %>%
  dplyr::summarize(num_abs = length(unique(ab_name)))
dfNumRefs = dfForPlot %>%
  group_by(strain_name, in_trial, ab_name) %>%
  dplyr::summarize(num_refs = length(unique(ref_name))) %>%
  group_by(strain_name, in_trial) %>%
  dplyr::summarize(num_refs = max(num_refs))

dfForPlot = dfForPlot %>%
  inner_join(dfNumAbs, by = c("strain_name", "in_trial")) %>%
  inner_join(dfNumRefs, by = c("strain_name", "in_trial"))
...
grid.arrange(grobs = plots, nrow = length(filterStrains))
```
