...
```{r mab-fold-fig, fig.height = 20, fig.width = 16, fig.retina = 4, message = FALSE}
strainTitle = c(
  "S:501Y" = "N501Y",
  "B.1.1.7 Spike" = "B.1.1.7",
  "S:484K" = "E484K",
...
mAbLevels = c(
  ...
  "Cilgavimab", "Tixagevimab", "Tixagevimab+\nCilgavimab", "Bamlanivimab+\nEtesevimab",
  "Vir-7831", "C121", "C135", "C144",
  "COV2-2479", "COV2-2832",
  "CC12.1", "BD23", "C119", "P2B-2F6"
)
...
dfForPlot = dfSuscResultsMAb %>%
  ...
  mutate(
    ab_name = factor(
      ab_name,
      levels = unique(c(mAbLevels, ab_name))
    ),
    ref_name = factor(
      ref_name,
...
dfForPlot = dfForPlot %>%
  ...
  merge(dfForPlot, by = "strain_name")
...
for (strain in filterStrains) {
  ...
  plot = plot + geom_text(
    ...
    data = dfPartial,
    position = position_dodge2(preserve = 'single', width = widthTimer * numAbs),
    size = 1.5,
    vjust = -0.5
  )
}
