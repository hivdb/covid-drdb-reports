...
### VP figure
```{r vp-figure-paper, fig.height=20, fig.retina=4, fig.width=20, message=FALSE, warning=FALSE, echo=FALSE}

plasmaDT = read.dbTable('figure_plasma_fold.csv', tables_dir=TABLEDIR)
...
grid.arrange(grobs = plots, ncol=3, heights=c(1,1,1.3))
# grid.arrange(grobs = plots)
```


```{r vp-figure-paper-facet, fig.height=20, fig.retina=4, fig.width=20, message=FALSE, warning=FALSE, echo=FALSE}
...
### Plasma titer
```{r plasma-titer, fig.height=20, fig.retina=4, fig.width=20, message=FALSE, warning=FALSE, echo=FALSE}

plasmaDT = read.dbTable('figure_plasma_titer.csv', tables_dir=TABLEDIR) %>%
  filter(month=='>6')
...
for (st in STUDY_TYPE) {
    dfPlot = plasmaDT %>%
      filter(rx_name == st) %>%
      filter(iso_name %in% VARIANT) %>%
      mutate(
        iso_name = factor(iso_name, VARIANT),
        level = factor(level, TITER_LEVEL)
      )

    dfAgg = dfPlot %>%
      group_by(iso_name, num_study) %>%
      summarise(total=sum(num_sample)) %>%
      mutate(
        total_label = ifelse(
          num_study > 1,
          yes=sprintf('%s\nstudies', num_study),
    ...
    p <- ggplot(data=dfPlot, aes(x=iso_name, y=num_sample)) +
      scale_x_discrete(labels=c(
        'Alpha' = 'Alpha (B.1.1.7)',
        'Beta' = 'Beta (B.1.351)',
        'Gamma' = 'Gamma (P.1)',
