...
df = read.dbTable('figure/variant_single_cp.csv', tables_dir=TABLEDIR)
df = df %>%
...
df = read.dbTable('figure/variant_combo_cp.csv', tables_dir=TABLEDIR)
df = df %>%
...
df = read.dbTable('figure/variant_single_vp.csv', tables_dir=TABLEDIR)
df = df %>%
  filter(pattern %in% INDIV_MUT_LIST) %>%
...
df = read.dbTable('figure/variant_combo_vp.csv', tables_dir=TABLEDIR)
df = df %>%
  filter(var_name %in% COMBO_MUT_LIST) %>%
  filter(vaccine %in% VACCINE_LIST) %>%
  mutate(
...
df = read.dbTable('dms/fold_dms.csv', tables_dir=TABLEDIR) %>%
  mutate(
...
df = read.dbTable('dms/dms_missing_fold.csv', tables_dir=TABLEDIR)
...
df = read.dbTable('variant/summary_single.csv', tables_dir=TABLEDIR) %>%
  mutate(
...
dfCP = read.dbTable('variant/summary_single_cp.csv', tables_dir=TABLEDIR)
dfCP = get_domain_table(dfCP, 'CP')
dfVP = read.dbTable('variant/summary_single_vp.csv', tables_dir=TABLEDIR)
dfVP = get_domain_table(dfVP, 'VP')
dfMAb = read.dbTable('variant/summary_single_mab.csv', tables_dir=TABLEDIR)
...
df = read.dbTable('variant/summary_combo.csv', tables_dir=TABLEDIR) %>%
  filter(
